...
> 教程类强制横屏（categories/tutorial.md orientation_hint=landscape）。B 站横屏播放，观众要读文字/对比双栏/数据卡/金句同屏，竖屏装不下。s6_assemble 横屏分支依赖此字段。

## tutorial_reveal_mode

**tutorial_reveal_mode: true**

> §6.16 教程横屏范式 PoC：一屏多区域 + reveal（**不是竖屏 phase 切换**）。每场景单 phase div + 多 region（标题/要点/数据/金句 grid 同屏）+ `data-reveal="N"` 按时间点依次淡入；区域 reveal 后保持同屏累积丰富。s6_assemble build_gsap 扫碎片 data-reveal 属性生成 reveal 动画。教程观众「读」画面消化，不被「切」着走。
...
| 数据强调 | `#FBBF24` | 金（+104% / 2 倍代码 / 76.6% 等数字高亮） |
| 对比强调 | `#10B981` | 绿（「是 / 正确」一栏 — 方法论、可落地） |
| 对比警示 | `#EF4444` | 红（「不是」一栏 — 工具测评、PPT 概念） |
| 中性 | `#94A3B8` | 灰（次要文字 / 注释 / 分隔） |

> 强色控制：金色仅数据高亮 / 绿仅「是」栏 / 红仅「不是」栏。一屏多区域靠色块区分语义，不靠强切换。
...
| 对比双栏（不是/是）| 「这是什么」场景 | 左红「不是」+ 右绿「是」对称卡片，图标+标题+说明三行 |
| 阶段/能力列表区 | 7 阶段缩略 / AI 能力清单 | 横向阶段条或胶囊标签 stagger 200ms 依次亮起 |
| 数据卡区 | 2 倍代码 / +104% | 金色大字 + 「相关性非因果」灰色小字注释 |
| 金句区 | 阶段判断 / 收尾承接 | 金句条（金/蓝边框 + 白字）fade-in，长停留 |
...
- 阶段/能力标签：横向 stagger 亮起（200ms 间隔）
